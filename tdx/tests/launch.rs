...
fn launch() {
    ...
    if check_extension(KVM_CAP_MEMORY_MAPPING) {
        ...
        // NOTE: this is failing so far at `kvm_mmu_map_tdp_page` with -EFAULT (bad address)
        // `kvm_tdp_mmu_page_fault is returning 1 (or RET_PF_RETRY) which means the CPU should
        // fault again on the address... which I think is why we are getting the -EFAULT... but
        // that really isn't clear yet still because I don't think we should be getting an -EFAULT
        // still if we are hitting that code path
        tdx_vm
